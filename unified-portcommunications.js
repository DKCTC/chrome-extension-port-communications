	...
	var _connections = {},
		...
		Port = function(obj){
			...
			this.connect = function(obj){
				...
				//set the name if not set already
			...
			var doConnect = function(_obj,_extID,reconnect){
				//reset the connected flag and the actual port reference
				_this.connected = false;
				_this.actual = false;
				//handles connecting to external chrome apps vs extensions
			...
			this.postMessage = function(msg,callback){
				...
				if(typeof(callback) == 'function' && !_this.isOnConnectListener){
					...
					_this.portMsgCallbacks[callbackID] = callback;
